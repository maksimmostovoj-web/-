...
test("Пользователь может зарегистрироваться используя email и пароль", async ({
  ...
  await page
    ...
    .fill("sniper101120251");
  await page.getByRole("textbox", { name: "Email" }).click();
  await page
    .getByRole("textbox", { name: "Email" })
    .fill("sniper101120251@ya.ru");
...
test1('Пользователь может зарегистрироваться используя email и пароль', async ({ page }) => {
    getRegistration(page, email, name, password, url);  
    await expect(page.getByRole('navigation')).toContainText(name);
});

test2('Пользователь может изменить свое имя в профиле', async ({ page }) => {
    getRegistration(page, email, name, password, url); 
